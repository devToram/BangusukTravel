...
import instabot
# from instabot.api import api_photo
...
def _print_bot_last_state(bot):
    ...
    pprint.pprint(api.api.last_response, indent=4)
    pprint.pprint(api.api.last_response.headers, indent=4)
    pprint.pprint(api.last_json, indent=4)
...
def _reset_challenge(_bot):
    ...
    try:
        _bot.api.send_request(reset_url, login=True)
...
def _solve_checkpoint_challenge(_bot):
    ...
    try:
        _bot.api.send_request(challenge_url, data, login=True)
    except Exception as e:
        _bot.logger.error(e)
        return False
...
try:
    api.login(use_cookie=False,ask_for_code=True)
    api.logger.info('User logged successfully, no Challenge required')
    api.upload_photo(photo="./capture-test.jpg", 
		caption = "testing" + "\r\n" + " #pythondeveloper #webdeveloper") 
    exit()
except Exception as e:
    api.logger.error(e)
    if api.last_json.get('error_type', '') == 'checkpoint_challenge_required':
        print("Checkpoint_challenge found, attempting to solve...")
        success = _solve_checkpoint_challenge(api)
        if success:
            api.login(cookie_fname=COOKIE_FNAME)
    else:
        print("Unknown challenge found, share the next output to get support")
        _print_bot_last_state(api)